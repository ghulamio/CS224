...
main:
	...
	move	$a0, $s0
	jal	getSparseMatrixSize
	
	#move	$s4, $v0 # size of the sparse matrix
	#move	$a0, $s4
	#move	$a1, $s2
	#jal	printArray
...
#****** subroutine Input Dimensions ******
# params[in]: Nan
# returns : $v0 = the value of N in an NxN matrix
inputDimension:	
...
## function printArray
## input: $a0 = size of the array, $a1 = base address of the array
## output: NaN	
printArray:
	# save S registers and RA on stack frame
	addi	$sp, $sp, -36
	...
	# the arguments passed are the base address of the array and the size of the array
	move	$s0, $a0 #size of the array
	move	$s1, $a1 # base address of the array
	
	move	$s2, $0 # start from index 0
	
	# used for the last comma removal only
	move	$s6, $s0
	addi	$s6, $s6, -1
	...
	printLoop:
		...
		lw	$a0, 0($s1)
		syscall
		beq	$s2, $s6, noLastComma
		li	$v0, 11
		li	$a0, ','
		syscall
		noLastComma:		
		addi $s1, $s1, 4	# increase base address for word
...
## ***** subroutine get sparse matrix size *****
# params : $a0 = N dimension
# returns : $v0 = size of sparse matrix
getSparseMatrixSize:
	# save registers on stack
	addi	$sp, $sp, -36
	sw	$s0, 0($sp)
